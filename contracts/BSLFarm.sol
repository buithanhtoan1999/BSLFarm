...
contract BSLFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. number of token reward to distribute per block.
    ...
    // The block number when token reward mining starts.
    ...
    // Update the given pool's token Reward allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    //Update number of reward token per block 
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardTokenPerShare = accRewardTokenPerShare.add(
                tokenRewardAmount.mul(1e12).div(lpSupply)
            );
        }
        return user.pendingReward.add(user.amount.mul(accRewardTokenPerShare).div(1e12).sub(user.rewardDebt));
    }
    ...
    function harvest(uint256 _pid) public validatePoolById(_pid) returns(uint256) {
       ...
       uint256 totalPending = 
                            user.amount.mul(pool.accRewardTokenPerShare)
                                        .div(1e12)
                                        .sub(user.rewardDebt)
                                        .add(pendingReward); 
       user.pendingReward = 0;
       if (totalPending > 0) {
            safeRewardTokenTransfer(msg.sender, totalPending); 
        }
        user.rewardDebt = user.amount.mul(pool.accRewardTokenPerShare).div(1e12);
    ...
    // Safe bsl transfer function, just in case if rounding error causes pool to not have enough reward token s.
    function safeRewardTokenTransfer(address _to, uint256 _amount) internal {
        uint256 rewardBal = IBEP20(rewardToken).balanceOf(address(this));
        if (_amount > rewardBal) {
            rewardToken.transfer(_to, rewardBal);
        } else {
            IBEP20(rewardToken).transfer(_to, _amount);
        }
    }
